...
def listar_avaliacao():
    # Iniciando a sessao e buscando a query
    session_db = Session()
    # como se fosse um Select *from .all = listar todos e mostrando na pagina listaravaliacao.html
    avaliacoes = session_db.query(Avaliacao).all()
    return render_template("listaravaliacao.html",avaliacoes=avaliacoes)

# desafio - mostrar a carinha personalizada 
